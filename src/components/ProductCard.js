...
export default function ProductCard({ children, filterBy, id, image }) {
    const needBg = false;
    ...
    let redirectPath = 'product-' + id;   // need to be string
    if(filterBy === PROMOTIONS) {
        redirectPath = `${filterBy.toLowerCase()}/redirectPath`
    }
